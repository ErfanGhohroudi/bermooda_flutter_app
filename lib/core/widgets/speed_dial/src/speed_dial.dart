...
class WSpeedDial extends StatefulWidget {
  ...
  @override
  State createState() => _WSpeedDialState();
}

class _WSpeedDialState extends State<WSpeedDial> with SingleTickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(final WSpeedDial oldWidget) {
  ...
  toggleOverlay() {
    if (_open) {
      ...
      overlayEntry = OverlayEntry(
        builder: (final ctx) => _ChildrensOverlay(
          widget: widget,
          dialKey: dialKey,
  ...
  Widget _renderButton() {
    var child = widget.animatedIcon != null
        ...
        : AnimatedBuilder(
            animation: _controller,
            builder: (final BuildContext context, final _) => Transform.rotate(
    ...
    var animatedFloatingButton = AnimatedBuilder(
      animation: _controller,
      builder: (final context, final ch) => CompositedTransformTarget(
          link: _layerLink,
          key: dialKey,
          child: AnimatedFloatingButton(
            ...
            heroTag: widget.heroTag != null ? '${widget.heroTag}-main-${widget.key?.hashCode ?? widget.hashCode}' : null,
            shape: widget.shape,
            child: child,
          )),
    );
...
class _ChildrensOverlay extends StatelessWidget {
  ...
  List<Widget> _getChildrenList() {
    return widget.children
        .map((final SpeedDialChild child) {
          ...
          return AnimatedChild(
            ...
            shape: child.shape,
            heroTag: widget.heroTag != null ? '${widget.heroTag}-child-$index-${widget.key?.hashCode ?? widget.hashCode}' : null,
