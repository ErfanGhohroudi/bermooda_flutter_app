...
import 'crm_statistics_controller.dart';

class CrmStatisticsPage extends StatefulWidget {
  ...
  @override
  State<CrmStatisticsPage> createState() => _CrmStatisticsPageState();
}

class _CrmStatisticsPageState extends State<CrmStatisticsPage> with CrmStatisticsController {
  @override
  void initState() {
    super.initState();
    initialController(category: widget.category);
  }
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text(
                '${s.overallStatistics} (${widget.category.title ?? "- -"})',
                style: context.theme.textTheme.titleMedium?.copyWith(
                  fontWeight: FontWeight.bold,
                ),
              ),
  ...
  Widget _buildUserStatisticsCard(final CrmUserStatisticsSummary stat) {
    // محاسبه درصد موفقیت کارشناس
    final successRate = stat.totalFollowUpsCount > 0 ? ((stat.successfulSaleCount / stat.totalFollowUpsCount) * 100).round() : 0;
  ...
  PerformanceStatusEntity _getPerformanceStatus(final CrmUserStatisticsSummary stat) {
    if (stat.totalFollowUpsCount == 0) {
    ...
    final successRate = (stat.successfulSaleCount / stat.totalFollowUpsCount) * 100;
    final delayedRate = (stat.delayedFollowUpsCount / stat.totalFollowUpsCount) * 100;
  ...
  List<StatCardValueEntity> _getStatCardValues({final CrmUserStatisticsSummary? stat}) {
    return [
      StatCardValueEntity(
        icon: Icons.headset_mic_rounded,
        title: s.followUps,
        value:
            stat?.totalFollowUpsCount.toString().separateNumbers3By3() ??
            totalStatistics.value.totalFollowUpsCount.toString().separateNumbers3By3(),
        color: Colors.blue,
        subtitle: s.totalFollowups,
      ),
      StatCardValueEntity(
        icon: Icons.pending_outlined,
        title: s.inProgress,
        value:
            stat?.followingUpsCount.toString().separateNumbers3By3() ??
            totalStatistics.value.followingUpsCount.toString().separateNumbers3By3(),
        color: Colors.orange,
        subtitle: '${s.followUps} ${s.inProgress}',
      ),
      StatCardValueEntity(
        icon: Icons.attach_money,
        title: s.closedWon,
        value:
            stat?.successfulSaleCount.toString().separateNumbers3By3() ??
            totalStatistics.value.successfulSaleCount.toString().separateNumbers3By3(),
        color: Colors.green,
        subtitle: s.closedWonFollowups,
      ),
      StatCardValueEntity(
        icon: Icons.schedule_outlined,
        title: s.overdue.toString().separateNumbers3By3(),
        value:
            stat?.delayedFollowUpsCount.toString().separateNumbers3By3() ??
            totalStatistics.value.delayedFollowUpsCount.toString().separateNumbers3By3(),
        color: Colors.red,
        subtitle: s.overdueFollowups,
      ),
      StatCardValueEntity(
        icon: Icons.timer_outlined,
        title: s.timeSpent,
        value: stat?.totalSeconds.toHoursMinutesSecondsFormat ?? totalStatistics.value.totalSeconds,
