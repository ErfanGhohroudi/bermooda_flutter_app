...
class ProjectBoardController extends GetxController {
  ...
  late final KanbanController<ProjectSectionReadDto, TaskReadDto> kanbanController;
  final bool haveAdminAccess = Get.find<PermissionService>().haveProjectAdminAccess;

  final Rx<Section<ProjectSectionReadDto, TaskReadDto>?> pendingList = Rx(null as Section<ProjectSectionReadDto, TaskReadDto>?);
  final Rx<bool> isWebSocketConnect = RxBool(true);
  final Rx<bool> showNewTaskField = RxBool(false);

  // سکشن مربوط به ایجاد سکشن جدید
  final _addNewSectionWidget = Section<ProjectSectionReadDto, TaskReadDto>(
    slug: 'add_section',
  ...
  void onInit() {
    super.onInit();
    kanbanController = KanbanController<ProjectSectionReadDto, TaskReadDto>(
    ...
    _wsSub = webSocketService.messages.listen((final jsonData) {
      ...
      final projectId = jsonData["data"]?['project_id']?.toString();
      // if event is not for this board
      if (project.id != projectId || queryType != ModuleType.project) return;
  ...
  void onClose() {
    ...
    debugPrint("ProjectBoardController closed!!!");
    super.onClose();
  }
  ...
  void _onAddOrUpdateSection(final Map<String, dynamic> data) async {
    ...
    final ProjectSectionReadDto? sectionData = sectionDataJson == null ? null : ProjectSectionReadDto.fromMap(sectionDataJson);
    if (sectionData == null || sectionSlug == null) return;
    final newSection = Section<ProjectSectionReadDto, TaskReadDto>(
  ...
  void _fetchBoardSections() {
    _kanbanDatasource.getAllBoardSectionsAndItems<ProjectSectionReadDto, TaskReadDto>(
      slug: project.id ?? '',
      requestType: KanbanRequestType.project_board,
      sectionFromMap: (final x) => ProjectSectionReadDto.fromMap(x),
      itemListFromMap: (final list) => list.map(_parseItemFromMap).toList(),
      onResponse: (final response) {
        List<Section<ProjectSectionReadDto, TaskReadDto>> sections = response;
  ...
  void _moveCard(
    final Item<TaskReadDto> item,
    final int fromSection,
    final int fromIndex,
    final int toSection,
    final int toIndex,
    final Section<ProjectSectionReadDto, TaskReadDto> targetSection,
  ) {
